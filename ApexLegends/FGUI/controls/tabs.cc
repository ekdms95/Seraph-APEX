...
void fgui::tabs::draw() {
	...
	if (m_parent_element)
	{
		if (fgui::tabs::set_style == fgui::tabs_layout::VERTICAL)
	...
	for (std::size_t i = 0; i < m_info.size(); i++) {
		...
		if (fgui::tabs::set_style == fgui::tabs_layout::HORIZONTAL)
			area = { a.x + (static_cast<int>(i) * tab_button_size), a.y, tab_button_size, m_height };
		else if (fgui::tabs::set_style == fgui::tabs_layout::VERTICAL)
